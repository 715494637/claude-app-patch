    ...
    patches: [
      {
        ...
        replace: `const{handleCoworkVMApi:w,cleanupVMBundleIfUnsupported:S}=await Promise.resolve().then(()=>hci);(function(_wc){try{let _dlStatus="not_downloaded",_runStatus="offline",_dlProgress=0;const _disp=FU.for(_wc).setImplementation({async download(){if(_dlStatus==="ready")return{success:true};_dlStatus="downloading";_dlProgress=0;try{_disp.dispatchDownloadStatusChanged("downloading")}catch(e){}const _tick=()=>new Promise(r=>setTimeout(r,120));for(let i=0;i<=100;i+=2){_dlProgress=i;try{_disp.dispatchDownloadProgress(i)}catch(e){}await _tick()}_dlStatus="ready";try{_disp.dispatchDownloadStatusChanged("ready")}catch(e){}return{success:true}},async startVM(n){if(_runStatus==="ready")return{success:true};_runStatus="booting";try{_disp.dispatchRunningStatusChanged("booting")}catch(e){}await new Promise(r=>setTimeout(r,3000));_runStatus="ready";try{_disp.dispatchRunningStatusChanged("ready")}catch(e){}return{success:true}},getDownloadStatus(){return _dlStatus},async getRunningStatus(){return _runStatus},isHostLoopModeEnabled(){return false},isHostLoopDevOverrideActive(){return false},setForceDisableHostLoop(n){},setYukonSilverConfig(n){},getInitialApiReachabilityState(){return{status:"reachable"}},async checkVirtualMachinePlatform(){return true},async enableVirtualMachinePlatform(){return true},async restartAfterVMPInstall(){},async deleteAndReinstall(){}});R.info("[Patch] Mock ClaudeVM IPC handlers registered")}catch(_e){R.error("[Patch] Mock ClaudeVM failed: "+_e)}})(e.webContents)`,
      },
    ],
  },
];
