...
| `scheme-3p-gateway.zip` | ⭐ **方案 1**：免登录，HTTPS 端点，不改文件（推荐） |
| `scheme-3p-http-patch.zip` | 方案 2：免登录，支持 HTTP 端点 |
| `scheme-full-patch.zip` | 方案 3：需登录，解锁隐藏功能 |
...
- **双击** `scheme-3p-gateway/setup.bat` → 自动读取配置 → 重启 Claude 即可
- **卸载**：双击 `scheme-3p-gateway/uninstall.bat`
...
- **双击** `scheme-3p-http-patch/setup.bat` → 自动构建便携版 + 配置 → 启动
- **卸载**：双击 `scheme-3p-http-patch/uninstall.bat`
...
- **适合**：有官方账号，想解锁额外功能的用户
- **双击** `scheme-full-patch/setup.bat` → 自动构建便携版 + 打补丁 → 启动
- **卸载**：双击 `scheme-full-patch/uninstall.bat`
...
```powershell
node scheme-3p-http-patch\setup.js --from-cli    # 不交互
node scheme-3p-http-patch\setup.js --status       # 查看状态
node scheme-3p-http-patch\setup.js --uninstall    # 卸载
```
